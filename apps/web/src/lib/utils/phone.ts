...
// Everything phone-related: the country calling codes the picker offers, the
// helpers that convert between a stored E.164 string and the picker's two
// fields, and the validation rules — which mirror the API
// (apps/api/src/utils/zod.util.ts).

/** ISO 3166-1 alpha-2 code, display name, and E.164 calling code. */
...
/**
 * Ordered alphabetically by name. India is the default selection (see
 * DEFAULT_COUNTRY) because that is the market this product serves, not because
...
/**
 * Splits a stored E.164 number back into a country and a local number so a
 * saved value can be shown in the picker.
 *
 * Dial codes are not prefix-free (+1 vs +1868, +7 for RU and KZ), so the
 * longest match wins and ties fall back to the first country listed.
 */
export function splitPhone(value: string | null | undefined): {
  ...
  if (!trimmed.startsWith('+')) return { country: DEFAULT_COUNTRY, national: trimmed.replace(/\D/g, '') }

  const match = [...COUNTRIES]
    .filter((c) => trimmed.startsWith(c.dial))
    .sort((a, b) => b.dial.length - a.dial.length)[0]

  if (!match) return { country: DEFAULT_COUNTRY, national: trimmed.replace(/\D/g, '') }

  return { country: match.code, national: trimmed.slice(match.dial.length).replace(/\D/g, '') }
}
...
/**
 * Mirrors the API rule in apps/api/src/utils/zod.util.ts: any country in E.164
 * form, with +91 numbers still held to the Indian mobile pattern.
 *
 * Blank is allowed and means "not provided" — the PhoneInput emits "" when the
 * number is cleared, and the API treats an empty string as absent.
 */
export const optionalPhone = z
  ...
  .refine(
    (v) => !v || !v.startsWith('+91') || /^\+91[6-9]\d{9}$/.test(v),
    'Enter a valid Indian mobile number (10 digits, starting 6-9)'
  )

/** Same rule, but the field must be filled in. */
